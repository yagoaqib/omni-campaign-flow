...
export const uploadAvatar = async (file: File, workspaceId: string): Promise<string> => {
  try {
...
export const processAndUploadAvatar = async (
  ...
  try {
    ...
    // Upload to storage
    return await uploadAvatar(finalFile, workspaceId);
  } catch (error) {
    console.error('Error processing and uploading avatar:', error);
    throw error;
  }
};
