...
-- 1) Helper functions for RLS: membership and roles
...
-- 2) Core tables: workspaces & members
...
create table if not exists public.phone_numbers (
  ...
  quality_rating text not null default 'UNKNOWN', -- HIGH|MEDIUM|LOW|UNKNOWN
...
create table if not exists public.templates (
  ...
  status text not null,                 -- APPROVED|REJECTED|PENDING|PAUSED
...
create table if not exists public.audience_items (
  ...
  e164 text,
  wa_id text,
  validation_status text not null default 'PENDING', -- VALID|INVALID|NO_WA|PENDING
...
create table if not exists public.dispatch_jobs (
  ...
  client_msg_id text,
  status text not null default 'QUEUED', -- QUEUED|SENT|DELIVERED|READ|FAILED|SKIPPED|RETRY
  error_code text,
...
create table if not exists public.webhook_events (
  id uuid primary key default gen_random_uuid(),
  workspace_id uuid not null references public.workspaces(id) on delete cascade,
  event_type text not null,      -- messages|quality_update|template_status_update
...
-- 9) Policies: "somente meu workspace"
-- Workspaces
create policy if not exists "workspaces_select_member"
  on public.workspaces for select to authenticated
  using (public.is_member(id));
create policy if not exists "workspaces_insert_any_auth"
  on public.workspaces for insert to authenticated
  with check (true);
create policy if not exists "workspaces_update_member"
  on public.workspaces for update to authenticated
  using (public.is_member(id)) with check (public.is_member(id));
create policy if not exists "workspaces_delete_owner_admin"
...
-- allow first member (workspace bootstrap) OR existing owner/admin to add
create policy if not exists "members_insert_self_or_admin"
...
-- WABAs
create policy if not exists "wabas_sel"
  on public.wabas for select to authenticated
  using (public.is_member(workspace_id));
create policy if not exists "wabas_ins"
  on public.wabas for insert to authenticated
  with check (public.is_member(workspace_id));
create policy if not exists "wabas_upd"
  on public.wabas for update to authenticated
  using (public.is_member(workspace_id)) with check (public.is_member(workspace_id));
create policy if not exists "wabas_del"
  on public.wabas for delete to authenticated
  using (public.is_member(workspace_id));

-- Phone Numbers
create policy if not exists "pn_sel"
  on public.phone_numbers for select to authenticated
  using (public.is_member(workspace_id));
create policy if not exists "pn_ins"
  on public.phone_numbers for insert to authenticated
  with check (public.is_member(workspace_id));
create policy if not exists "pn_upd"
  on public.phone_numbers for update to authenticated
  using (public.is_member(workspace_id)) with check (public.is_member(workspace_id));
create policy if not exists "pn_del"
  on public.phone_numbers for delete to authenticated
  using (public.is_member(workspace_id));

-- Templates
create policy if not exists "tpl_sel"
  on public.templates for select to authenticated
  using (public.is_member(workspace_id));
create policy if not exists "tpl_ins"
  on public.templates for insert to authenticated
  with check (public.is_member(workspace_id));
create policy if not exists "tpl_upd"
  on public.templates for update to authenticated
  using (public.is_member(workspace_id)) with check (public.is_member(workspace_id));
create policy if not exists "tpl_del"
  on public.templates for delete to authenticated
  using (public.is_member(workspace_id));

-- Template Stacks
create policy if not exists "tst_sel"
  on public.template_stacks for select to authenticated
  using (public.is_member(workspace_id));
create policy if not exists "tst_ins"
  on public.template_stacks for insert to authenticated
  with check (public.is_member(workspace_id));
create policy if not exists "tst_upd"
  on public.template_stacks for update to authenticated
  using (public.is_member(workspace_id)) with check (public.is_member(workspace_id));
create policy if not exists "tst_del"
  on public.template_stacks for delete to authenticated
  using (public.is_member(workspace_id));

-- Template Stack Items (derive via stack)
create policy if not exists "tsi_sel"
  on public.template_stack_items for select to authenticated
  using (exists (
    select 1 from public.template_stacks ts
    where ts.id = stack_id and public.is_member(ts.workspace_id)
  ));
create policy if not exists "tsi_ins"
  on public.template_stack_items for insert to authenticated
  with check (exists (
    select 1 from public.template_stacks ts
    where ts.id = stack_id and public.is_member(ts.workspace_id)
  ));
create policy if not exists "tsi_upd"
  on public.template_stack_items for update to authenticated
  using (exists (
    select 1 from public.template_stacks ts
    where ts.id = stack_id and public.is_member(ts.workspace_id)
  )) with check (exists (
...
create policy if not exists "tsi_del"
  on public.template_stack_items for delete to authenticated
  using (exists (
    select 1 from public.template_stacks ts
    where ts.id = stack_id and public.is_member(ts.workspace_id)
  ));
...
-- Audiences
create policy if not exists "aud_sel"
  on public.audiences for select to authenticated
  using (public.is_member(workspace_id));
create policy if not exists "aud_ins"
  on public.audiences for insert to authenticated
  with check (public.is_member(workspace_id));
create policy if not exists "aud_upd"
  on public.audiences for update to authenticated
  using (public.is_member(workspace_id)) with check (public.is_member(workspace_id));
create policy if not exists "aud_del"
  on public.audiences for delete to authenticated
  using (public.is_member(workspace_id));

-- Audience Items (derive via audience)
create policy if not exists "ai_sel"
  on public.audience_items for select to authenticated
  using (exists (
    select 1 from public.audiences a
    where a.id = audience_id and public.is_member(a.workspace_id)
  ));
create policy if not exists "ai_ins"
  on public.audience_items for insert to authenticated
  with check (exists (
    select 1 from public.audiences a
    where a.id = audience_id and public.is_member(a.workspace_id)
  ));
create policy if not exists "ai_upd"
  on public.audience_items for update to authenticated
  using (exists (
    select 1 from public.audiences a
    where a.id = audience_id and public.is_member(a.workspace_id)
  )) with check (exists (
...
create policy if not exists "ai_del"
  on public.audience_items for delete to authenticated
  using (exists (
    select 1 from public.audiences a
    where a.id = audience_id and public.is_member(a.workspace_id)
  ));
...
-- Campaigns
create policy if not exists "cmp_sel"
  on public.campaigns for select to authenticated
  using (public.is_member(workspace_id));
create policy if not exists "cmp_ins"
  on public.campaigns for insert to authenticated
  with check (public.is_member(workspace_id));
create policy if not exists "cmp_upd"
  on public.campaigns for update to authenticated
  using (public.is_member(workspace_id)) with check (public.is_member(workspace_id));
create policy if not exists "cmp_del"
  on public.campaigns for delete to authenticated
  using (public.is_member(workspace_id));

-- Campaign Numbers (derive via campaign)
create policy if not exists "cn_sel"
  on public.campaign_numbers for select to authenticated
  using (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  ));
create policy if not exists "cn_ins"
  on public.campaign_numbers for insert to authenticated
  with check (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  ));
create policy if not exists "cn_upd"
  on public.campaign_numbers for update to authenticated
  using (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  )) with check (exists (
...
create policy if not exists "cn_del"
  on public.campaign_numbers for delete to authenticated
  using (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  ));

-- Campaign Template Stacks (derive via campaign)
create policy if not exists "cts_sel"
  on public.campaign_template_stacks for select to authenticated
  using (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  ));
create policy if not exists "cts_ins"
  on public.campaign_template_stacks for insert to authenticated
  with check (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  ));
create policy if not exists "cts_upd"
  on public.campaign_template_stacks for update to authenticated
  using (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  )) with check (exists (
...
create policy if not exists "cts_del"
  on public.campaign_template_stacks for delete to authenticated
  using (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  ));

-- Dispatch Jobs (readable by members; writes only by Edge with service role)
create policy if not exists "dj_sel"
  on public.dispatch_jobs for select to authenticated
  using (exists (
    select 1 from public.campaigns c
    where c.id = campaign_id and public.is_member(c.workspace_id)
  ));
...
-- Webhook Events (readable by members; writes only by Edge)
create policy if not exists "wh_sel"
