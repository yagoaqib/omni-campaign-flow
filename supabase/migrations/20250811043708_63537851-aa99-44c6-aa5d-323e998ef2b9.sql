-- Fresh migration v2: create tables first, then helper functions, then policies
create extension if not exists pgcrypto;

-- Tables: workspaces & members first
...
-- Workspaces
alter table public.workspaces disable row level security; -- temporarily to avoid dependency on functions during creation
alter table public.workspaces enable row level security;
