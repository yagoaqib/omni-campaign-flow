-- Secure Contacts, Audiences and Audience Items with RLS and proper policies (fixed quoting)
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY ai_sel ON public.audience_items FOR SELECT USING (EXISTS (SELECT 1 FROM public.audiences a WHERE a.id = audience_items.audience_id AND is_member(a.workspace_id)));';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='audience_items' AND policyname='ai_ins'
  ) THEN
    EXECUTE 'CREATE POLICY ai_ins ON public.audience_items FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.audiences a WHERE a.id = audience_items.audience_id AND is_member(a.workspace_id)));';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='audience_items' AND policyname='ai_upd'
  ) THEN
    EXECUTE 'CREATE POLICY ai_upd ON public.audience_items FOR UPDATE USING (EXISTS (SELECT 1 FROM public.audiences a WHERE a.id = audience_items.audience_id AND is_member(a.workspace_id))) WITH CHECK (EXISTS (SELECT 1 FROM public.audiences a WHERE a.id = audience_items.audience_id AND is_member(a.workspace_id)));';
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='audience_items' AND policyname='ai_del'
  ) THEN
    EXECUTE 'CREATE POLICY ai_del ON public.audience_items FOR DELETE USING (EXISTS (SELECT 1 FROM public.audiences a WHERE a.id = audience_items.audience_id AND is_member(a.workspace_id)));';
  END IF;
END $$;
